...
<Callout type="tip" title="This is fully implemented">
  Earlier docs claimed passkeys returned <code>501 Not Implemented</code>. That is no
...
<APIEndpoint method="DELETE" path="/v1/passkeys/{id}" title="Revoke a passkey" />

## Passkeys as a second factor

The same registered passkeys can be reused as a **WebAuthn second factor** during
step-up MFA — see [MFA → WebAuthn as a second factor](/docs/mfa#webauthn-as-a-second-factor).
