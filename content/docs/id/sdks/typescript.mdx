...
`${QEETID_APP_URL}` as a post-logout URI on your [OIDC client](/id/oidc/clients).
...
<Callout type="info">
  How it splits: the Edge middleware uses Web Crypto only (no Node code in the
  Edge bundle); <code>auth()</code> runs in the Node runtime and verifies the
  ES256 token via
  <code>@qeetid/sdk</code>. Pair with <a href="/id/sdks/react">@qeetid/react</a>{" "}
  for UI.
</Callout>
